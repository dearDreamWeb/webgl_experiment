export default function drawPolygon(gl: WebGLRenderingContext, program: WebGLProgram, scale: number = 0, rotate: number = 0, translateX = 0, translateY = 0) {
 ...
 * 得到三角形的点数据
 */
function getShape(center: number[], r: number, n: number, scale: number = 0, rotate: number = 0): number[] {
    let idx = 0;
    let x = 0;
    let y = 0;
    let radius = 0;
    ...
    for (idx = 0; idx < n; idx++) {
        res.push(
            ...getPointArr(pointArr[idx], pointArr[(idx + 1) % n], center)
        ); // 注意这里是逆时针排布的
    }
    return res;
}
...
* 更新数据
*/
export function updateFn(gl: WebGLRenderingContext, program: WebGLProgram, n: number, scale: number, rotate: number, xDistance = 0, yDistance = 0) {
