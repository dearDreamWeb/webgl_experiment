...
// 根据点来绘制图形
export function draw(gl: WebGLRenderingContext, paramPoints: number[], type: string) {
    const vertex = `
      attribute vec2 position;
...
export function parametric(gl: WebGLRenderingContext, xFunc: (t: number, ...args: any) => number, yFunc: (t: number, ...args: any) => number) {
